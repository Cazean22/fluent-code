...
fn transcript_items_round_trip_exact_sessions() {
    run_exact_app_libtest("session::store::tests::transcript_items_round_trip_exact_sessions");
}

#[test]
fn transcript_items_synthesize_legacy_sessions_as_approximate() {
    run_exact_app_libtest(
        "session::store::tests::transcript_items_synthesize_legacy_sessions_as_approximate",
    );
}

fn run_exact_app_libtest(module_qualified_test_name: &str) {
    let cargo = std::env::var("CARGO").unwrap_or_else(|_| "cargo".to_string());
    let isolated_target_dir = unique_temp_dir("fluent-code-app-task2-exact-wrapper");
